...
lmconclusion <- function(dataset) {
  if(lmfit(dataset)[4] < 0.05) {
    paste("Conclusion: Since the p-value is less than 0.05,  i.e. 0 does not lie withinn the 95% confidence interval for Beta, it is statistically unlikely that the null hypothesis is correct, and so We reject the null hypothesis in favour of the alternative hypothesis, and assume that Beta =", lmfit(dataset)[4], ".", sep = "n")
  } else {
